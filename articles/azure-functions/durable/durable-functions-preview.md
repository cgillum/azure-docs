...
The interfaces used to generate typed proxies have the following limitations:

* The type parameter for `SignalEntityAsync<T>` must be an interface.
...
In most cases, interfaces which do not meet these requirements will result in a runtime exception.
